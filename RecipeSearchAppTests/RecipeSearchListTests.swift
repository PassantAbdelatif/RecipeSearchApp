//
//  RecipeSearchAppTests.swift
//  RecipeSearchAppTests
//
//  Created by Passant Abdelatif on 02/11/2022.
//

import XCTest
@testable import RecipeSearchApp

class RecipeSearchListTests: XCTestCase {
    
    var presenter: RecipeSearchPresenter!
    var view = RecipeSearchListMockView()
    var interactor = RecipeSearchListMockInteractor()

    override func setUp() {
        presenter = RecipeSearchPresenter()
        presenter.recipesSearchView = view
        presenter.recipesSearchInteractor = interactor
        interactor.presenter = presenter
    }
    
    override func tearDownWithError() throws {
        presenter = nil
    }

    func testExample() throws {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
        // Any test you write for XCTest can be annotated as throws and async.
        // Mark your test throws to produce an unexpected failure when your test encounters an uncaught error.
        // Mark your test async to allow awaiting for asynchronous code to complete. Check the results with assertions afterwards.
    }

    func testPerformanceExample() throws {
        // This is an example of a performance test case.
        self.measure {
            // Put the code you want to measure the time of here.
            
        }
    }

}


class RecipeSearchListMockView: PresenterToViewRecipesProtocol {
    func startViewLoader() {
        
    }
    
    func endViewLoader() {
        
    }
    
    func sendDataToView(recipeList: [Hits]) {
        
    }
    
    func sendErrorToView(error: String) {
        
    }
    
    func sendHealthCategoriesDataToView(healthLabels: [String]) {
        
    }
    
    func sendSavedSearchStringsToView(savedSearchStrings: [String]) {
        
    }
    
  
}

